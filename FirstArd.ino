...
int binarioParaDecimal() {
  ...
  entrada = converteDecimal(a, b, c, d, e); // Serial.println(entrada);
  
  saida = converteDecimal(f, g, h, i, j); // Serial.println(saida);
  
  calculo = saida - hora;   // Serial.println(calculo);
  return calculo;
}
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (hora >= 0 && hora <= 23 && minuto >= 0 && minuto <= 59 && segundo >= 0 && segundo <= 59) {
      Serial.println("Hora atual:"); // Exibe a hora formatada no monitor serial
      printHora();
